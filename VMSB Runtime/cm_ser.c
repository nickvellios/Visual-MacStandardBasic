...
short SER_Open(short portNum, short parms) {
    ...
    switch (portNum) {
        case 1:
            ...
            if (err == -1) {
                ErrHandler(1022, "\pin SerOpen command");
                return (-1);
            }
            ...
            SerBuf1 = NewPtr(1000);
            if (!SerBuf1) {
                ErrHandler(1001, "\pin SerOpen command");
                return (-1);
            }
...
short SER_SetParm(short portNum, short parms) {
    ...
    switch (portNum) {
        case 1:
            if (SerInA != -1 || SerOutA != -1) {
                ErrHandler(1026, "\pin SerSet command");
                return (-1);
            }
            // Setup baud etc...
            err = SerReset(SerInA, parms);
            if (err == -1) {
                ErrHandler(1022, "\pin SerSet command");
                return (-1);
            }
            err = SerReset(SerOutA, parms);
            if (err == -1) {
...
short SER_Write(short portNum, unsigned char *buf, long buflen) {
    ...
    switch (portNum) {
        case 1:
            if (SerOutA == -1) {
                ErrHandler(1026, "\pin SerWrite command");
                return (-1);
            }
...
long SER_Read(short portNum, unsigned char *buf, long numToRead) {
    ...
    switch (portNum) {
        case 1:
            if (SerInA == -1) {
                ErrHandler(1026, "\pin SerRead function");
                return (-1);
            }
