...
short SPR_SpriteFrame(short WinNum) {
    ...
    a = LockPixels(pmap);
    if (a == false) {
        ErrHandler(1001, "\p in SpriteFrame command");
        return (-4);
    }
    for (a = 0; a < 500; a++) {
...
short SPR_SpriteLoad(short ResID) {
    ...
    if (thePict == nil) {
        ErrHandler(1019, "\p in SpriteLoad command");
        return (-1);
    ...
    err = NewGWorld(&gwp, 0, &theRect, nil, nil, noNewDevice);
    if (err) {
        ErrHandler(1001, "\p in SpriteLoad command");
        return (-3);
    }// error allocating gworld
    pmap = GetGWorldPixMap(gwp);
    a = LockPixels(pmap);
    if (a == false) {
        ErrHandler(1001, "\p in SpriteLoad command");
    ...
    for (a = 0; a < 500; a++) {
        if (!SpriteImages[a].imageType) {
            ...
            SpriteImages[a].theRect = theRect;
            theBlackColor.red = 0;
            theBlackColor.green = 0;
            theBlackColor.blue = 0;
            theWhiteColor.red = 0xffff;
            theWhiteColor.green = 0xffff;
            theWhiteColor.blue = 0xffff;
...
short SPR_SpriteDup(short image1, short image2) {
    ...
    if (!SpriteImages[image1].imageType || !SpriteImages[image2].imageType) {
        ErrHandler(1041, "\p in SpriteDup command");
        return (-1);
    } // No image to draw
    r1 = SpriteImages[image1].theRect;
    r2 = SpriteImages[image2].theRect;
    CopyBits((BitMap * )(*SpriteImages[image1].thePixMap), (BitMap * )(*SpriteImages[image2].thePixMap), &r1, &r2,
             srcCopy, nil);
    return 0;
}
...
short SPR_SpriteDrawCopy(short imageNum, short x, short y, short priority, short tmode) {
    ...
    if (!SpriteImages[imageNum].imageType) {
        ErrHandler(1019, "\p in SpriteLoad command");
        return (-1);
    } // No image to draw
...
short SPR_SpriteDisplay(short frameNum) {
    ...
    if (frameNum < 0 || frameNum > 499) {
        ErrHandler(1041, "\p in SpriteDisplay command");
        return (-1);
    }
    if (!SpriteImages[frameNum].imageType) {
        ErrHandler(1041, "\p in SpriteDisplay command");
        return (-1);
    }  // No image to draw
...
short SPR_SpriteClear(short imageNum) {
    if (imageNum >= 2000) imageNum -= 2000;    //Fix offset
    if (!SpriteImages[imageNum].imageType) {
        ErrHandler(1041, "\p in SpriteClear command");
        return (-1);
    } // No image to erase
...
short SPR_SpriteWidth(short imageNum) {
    if (imageNum >= 2000) imageNum -= 2000;    //Fix offset
    if (!SpriteImages[imageNum].imageType) {
        ErrHandler(1041, "\p in SpriteWidth function");
        return (-1);
    } // No image to erase
...
short SPR_SpriteHeight(short imageNum) {
    if (imageNum >= 2000) imageNum -= 2000;    //Fix offset
    if (!SpriteImages[imageNum].imageType) {
        ErrHandler(1041, "\p in SpriteHeight function");
        return (-1);
    } // No image to erase
