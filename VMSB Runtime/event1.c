    ...
    for (a = 0; a < 500; a++) {
        if (CtlData[a].status == 4) {
            if (CtlData[a].theControl == theControl) {
                ...
                for (a = 0; a < 500; a++) {
                    if (CtlData[a].status == 4) {
                        if (WinNum == CtlData[a].WinNum && groupID == CtlData[a].lastItem &&
                            !(*CtlData[a].theControl)->contrlHilite) {
                                ...
                                switch (TNW) {
                                    case /*kControlCheckBoxPart*/11:
                                        if (!RadioCheck(theControl)) {
                                            ...
                                            EventCode[0] = 3;
                                            EventCode[1] = CtlData[TNH].WinNum;
                                            EventCode[2] = TNH;
                                            EventCode[3] = 0;
                                        }
                                        break;
                            ...
                            if (PtInRect(theEvent.where, &CtlData[a].theRect)) {
                                switch (CtlData[a].status) {
                                    ...
                                    case 15: // Listbox
                                        RGBForeColor(&CtlData[a].ForeRGB);
                                        RGBBackColor(&CtlData[a].BackRGB);
                                        if (CtlData[a].lastItem & 1)
                                            b = LClick(theEvent.where, theEvent.modifiers, CtlData[a].lt);
                                    ...
                                    case 25: // Icon
                                        ...
                                        if (cmdcode) {
                                            ...
                                            PlotIconID(&CtlData[a].theRect, atAbsoluteCenter, ttOffline,
                                                       CtlData[a].lastItem);
                                            Delay(10, &tc);
                                            EraseRect(&CtlData[a].theRect);
                                            tRect = CtlData[a].theRect;
                                            OffsetRect(&CtlData[a].theRect, -2, -2);
                                            PlotIconID(&CtlData[a].theRect, atAbsoluteCenter, ttNone,
                                                       CtlData[a].lastItem);
                                            InvalRect(&tRect);
                                            EventCode[0] = 9;
                                            EventCode[1] = CtlData[a].WinNum;
...
short HandleKeyDown(EventRecord theEvent) {
    ...
    if ((theEvent.modifiers & cmdKey) != 0) {
        ...
        if (mn != 0) {
            ...
            if (CmdKeys[theChar] >= 1000) {
                cmdcode = CmdKeys[theChar];
                return (cmdcode);
            }
            switch (theChar) {
                case 'c':    // copy
        ...
        if (ActiveTE == nil) // Check for single line
        {
            ...
            if (theChar == 13) {
                ...
                if (CtlData[d].opt1) {
                    ...
                    c = 0;
                    for (a = d + 1; a < 500; a++)
                        if (CtlData[a].WinNum == b && CtlData[a].enabled &&
                            (CtlData[a].status == 10 || CtlData[a].status == 11)) {
                            c++;
                            theChar = 9;
                            break;
                        }
                    if (!c) {   // Search for default button
                        for (a = 0; a < 500; a++)
                            if (CtlData[a].WinNum == b && CtlData[a].enabled && CtlData[a].status == 1 &&
                                CtlData[a].opt1) {
                                //Activate command button
                                EventCode[0] = 2;
                                EventCode[1] = CtlData[a].WinNum;
            ...
            switch (theChar) {
                case 9:
                    ...
                    if (b >= 0) {
                        if ((theEvent.modifiers & shiftKey) == 0) {  // No Shift Key
                            // Continue on from next
                            for (a = b + 1; a < 500; a++)
                                if (CtlData[a].WinNum == c && CtlData[a].enabled &&
                                    (CtlData[a].status == 10 || CtlData[a].status == 11)) {
                                    RGBForeColor(&CtlData[a].ForeRGB);
                                    RGBBackColor(&CtlData[a].BackRGB);
                                    TEDeactivate(ActiveTE);
                        ...
                        } else {     // Shift down so go backwards
                            for (a = b - 1; a >= 0; a--)
                                if (CtlData[a].WinNum == c && CtlData[a].enabled &&
                                    (CtlData[a].status == 10 || CtlData[a].status == 11)) {
                                    RGBForeColor(&CtlData[a].ForeRGB);
                ...
                case 127:  // del
                    if (ActiveTE != nil) {
                        ...
                        if (a >= 0) {
                            ...
                            RGBBackColor(&CtlData[a].BackRGB);
                            if (!(CtlData[a].lastItem & 2)) {
                                if (((*CtlData[a].te)->selStart == (*CtlData[a].te)->selEnd) &&
                                    (*CtlData[a].te)->selStart < (*CtlData[a].te)->teLength) {
