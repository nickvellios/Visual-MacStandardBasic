...
short FORM_Create(short FormID) {
    ...
    for (a = 0; a < RunHdr.NumOfProps; a++)
        if (RunVar.PropTable[a].FormID == FormID && RunVar.PropTable[a].CtlID == -1) {
            b = a;
            break;
        }
    if (b == -1) { return 0; }
        ...
        if (RunVar.PropTable[a].FormID == FormID && RunVar.PropTable[a].CtlID >= 1000) {
            ...
            switch (RunVar.PropTable[a].CtlType) {
                ...
                case 3: // Textbox
                    ...
                    RGBForeColor(&RunVar.PropTable[a].ForeRGB);
                    RGBBackColor(&RunVar.PropTable[a].BackRGB);
                    d = CTL_Textbox(WinNum, c[0], c[1], c[2], c[3], m1, RunVar.PropTable[a].value, text1,
                                    RunVar.PropTable[a].procID);
                    if (d >= 0) {
                ...
                case 4: // Icon
                    ControlVisibleFlag = RunVar.PropTable[a].visible;
                    d = CTL_LgIcon(WinNum, c[0], c[1], RunVar.PropTable[a].width, RunVar.PropTable[a].height,
                                   RunVar.PropTable[a].value, RunVar.PropTable[a].procID);
                    break;
                case 5: // Picture
                    ControlVisibleFlag = RunVar.PropTable[a].visible;
                    d = CTL_Picture(WinNum, c[0], c[1], c[2], c[3], RunVar.PropTable[a].value,
                                    RunVar.PropTable[a].procID);
                    break;
                case 6: // CheckBox
                    Str255ToStr(RunVar.PropTable[a].text, text1);
                ...
                case 7: // Radio
                    ...
                    d = CTL_Radio(WinNum, c[0], c[1], c[2], c[3], RunVar.PropTable[a].opt19, text1,
                                  RunVar.PropTable[a].procID);
                    if (d >= 0) {
                        if (RunVar.PropTable[a].value) SetControlValue(CtlData[d].theControl, 1);
                    }
                ...
                case 10: // Timer new 3.0
                    d = CTL_Timer(WinNum, RunVar.PropTable[a].value, RunVar.PropTable[a].enabled,
                                  RunVar.PropTable[a].procID);
                    break;
                case 11: // ScrollBar
                    ControlVisibleFlag = RunVar.PropTable[a].visible;
