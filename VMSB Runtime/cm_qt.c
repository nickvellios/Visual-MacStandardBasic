...
short QT_OpenMovie( short WinNum, Str255 MovieFilename, short CtlFlag)
{
	...
	if( !MSB_QTAvail ) { ErrHandler( 1034, nil); return( -99); }
	if( !WinData[WinNum].status) { ErrHandler( 1009, nil); return( -1); }
	
	for(a=0;a<10; a++) if( !MovieData[a].status ) {MovieNum=a; break;}
	if( MovieNum==-1 ) { ErrHandler( 1035, nil); return( -2); }

	theError = FSMakeFSSpec( 0, 0, MovieFilename,  &MovieData[MovieNum].fileSp );  
	if( theError ) {ErrHandler( 1016, nil); return( -2); }
...
short QT_PlayMovie( short MovieNum )
{
	...
	if(	!MovieData[MovieNum].status ) { ErrHandler( 1016, nil); return( -1); }
