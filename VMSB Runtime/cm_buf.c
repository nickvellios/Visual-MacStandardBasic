...
void MemFill(long bufferID, long start, long nob, short value) {
    ...
    unsigned char c, *ptr;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemFill command");
        return;
    }
    b = start + nob;
...
short MemInt(long bufferID, long location, short value) {
    ...
    short *ptr;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemInt command");
        return (-1);
    }
    if (location < 0 || location >= BufferSize[bufferID]) {
        ErrHandler(1003, "\pin MemInt command");
        return (-1);
    }
    HLock(Buffers[bufferID]);
...
short MemLong(long bufferID, long location, long value) {
    ...
    long *ptr;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemLong command");
        return (-1);
    }
    if (location < 0 || location >= BufferSize[bufferID]) {
        ErrHandler(1003, "\pin MemLong command");
        return (-1);
    }
    HLock(Buffers[bufferID]);
    tbuf = (*Buffers[bufferID]) + location;
...
short MemSingle(long bufferID, long location, short double value) {
    ...
    short double *ptr;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemSingle command");
        return (-1);
    }
    if (location < 0 || location >= BufferSize[bufferID]) {
        ErrHandler(1003, "\pin MemSingle command");
        return (-1);
    }
    HLock(Buffers[bufferID]);
    tbuf = (*Buffers[bufferID]) + location;
...
short MemDouble(long bufferID, long location, short double value) {
    ...
    short double *ptr;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemDouble command");
        return (-1);
    }
    if (location < 0 || location >= BufferSize[bufferID]) {
        ErrHandler(1003, "\pin MemDouble command");
        return (-1);
    }
    HLock(Buffers[bufferID]);
    tbuf = (*Buffers[bufferID]) + location;
...
void MemString(long bufferID, long location) {
    ...
    unsigned char *ptr;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemString command");
        return;
    }
...
short GetMemInt(long bufferID, long location) {
    ...
    short *ptr, value;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemInt function");
        return (-1);
    }
    if (location < 0 || location + 1 >= BufferSize[bufferID]) {
        ErrHandler(1003, "\pin MemInt function");
        return (-1);
    }
    HLock(Buffers[bufferID]);
    tbuf = (*Buffers[bufferID]) + location;
...
long GetMemLong(long bufferID, long location) {
    ...
    long *ptr, value;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemLong function");
        return (-1);
    }
    if (location < 0 || location + 3 >= BufferSize[bufferID]) {
        ErrHandler(1003, "\pin MemLong function");
        return (-1);
    }
    HLock(Buffers[bufferID]);
    tbuf = (*Buffers[bufferID]) + location;
...
short double GetMemSingle(long bufferID, long location) {
    ...
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemSingle function");
        return (-1);
    }
    if (location < 0 || location + 3 >= BufferSize[bufferID]) {
        ErrHandler(1003, "\pin MemSingle function");
        return (-1);
    }
    HLock(Buffers[bufferID]);
    tbuf = (*Buffers[bufferID]) + location;
...
short double GetMemDouble(long bufferID, long location) {
    ...
    short double *ptr, value;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemDouble function");
        return (-1);
    }
    if (location < 0 || location + 7 >= BufferSize[bufferID]) {
        ErrHandler(1003, "\pin MemDouble function");
        return (-1);
    }
    HLock(Buffers[bufferID]);
    tbuf = (*Buffers[bufferID]) + location;
...
short GetMemString(long bufferID, long location, long ln) {
    ...
    unsigned char *ptr;
    if (Buffers[bufferID] == nil) {
        ErrHandler(1002, "\pin MemString function");
        return (-1);
    }
    b = ln;
