...
short SND_Load( short theResID )
{
	...
	if(!MSB_SndMgrVer) { ErrHandler( 1037, nil ); return(-1); }
   	for(a=0;a<100;a++) if( MSB_SoundHandle[a] == nil ) {b=a; break;}
   	if( b == -1) { ErrHandler( 1038, nil ); return(-1); }
...
short  SND_Play( short sndNum )
{
	...
	if(!MSB_SndMgrVer){ ErrHandler( 1037, nil ); return(-1); }
   	for(a=0;a<100;a++) if( MSB_SoundChannel[a] == nil ) { b=a; break;}
   	if( b == -1) { ErrHandler( 1038, nil ); return(-1); }
    ...
    if ( MSB_SoundChannel[b] == nil ) { ErrHandler( 1018, nil ); return(-1); }
