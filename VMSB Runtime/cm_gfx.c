...

{
	...
	if(!WinData[WinNum].status) { ErrHandler( 1009, "\p in WinRedraw command" ); return(-1); }
...

{
	...
	for(a=0;a<100;a++) if(!WinData[a].status) { b=a; break; }
	...
	if( WinType >= 8000 ) { WinType -=8000; wc = 1; }

	if( WinType >= 4000 ) { WinType -=4000; CenterFlag = true; }

	if( WinType >= 2000 ) { WinType -=2000; ViewFlag = false; }

	if( WinType >= 1000 ) { WinType -=1000; Cbox = true; }
	...
	{	// Center WIndow on screen
		...
		w = GFX_Screeninfo(3) - wide;	if( w<0 ) w = 0;

		h = GFX_Screeninfo(4) - ht; if( h<0 ) h = 0;
...

{
	...
	if(	!WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinHide command" ); return(-1); }
...

{
	...
	if(	!WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinShow command" ); return(-1); }
...

{
	...
	if( !WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinSize command" ); return(-1); }
...

{
	...
	if(	!WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinMove command" ); return(-1); }
...

{
	...
	if(	!WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinOrigin command" ); return(-1); }
...

{
	...
	if( palNum<0 || palNum>9 ) { ErrHandler( 1006, "\p in LoadPalette command" ); return(-1); }
...

{
	...
	if( !WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinPalette command" ); return(-1); }

	if( !WinPal[palNum].status ) { ErrHandler( 1049, "\p in WinPalette command" ); return(-1); }

	palNum--;

	if( palNum<0 || palNum>9 ) { ErrHandler( 1006, "\p in WinPalette command" ); return(-1); }
...

{
	...
	if( !WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinClip command" ); return(-1); }
...

{
	...
	if(	!WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinTitle command" ); return(-1); }
...

{
	...
	if(	!WinData[WinNum].status ) { ErrHandler( 1009, "\p in WinBorder function" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in DrawText command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in DrawFont command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in DrawSize command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in DrawStyle command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in MoveTo command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in Point command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in LineTo command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in Line command" ); return(-1); }
...

{
...

		{
			...
			if( openPictureFlag ) 	{ b = 0; SetPort(openPictureWindow); }

			else

			{

				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in Box command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in Circle command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in Ellipse command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in DrawPicture command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in PenSize command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in PenPattern command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in PenMode command" ); return(-1); }
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in PenNormal command" ); return(-1); }
...

{
...

		{
			...
			// Need to allocate some new space
			 ...
			 if( WinData[WinNum].drawCmds == nil ) { ErrHandler( 1001, "\p"); return; }
...

		{
...

			 {
				...
				if( MemError() ) { ErrHandler( 1001, "\p"); return; }
...

{
...

		{
			...
			case 2:
				...
				rgb.red = 0xfc00; rgb.green = 0xf37d; rgb.blue = 0x052f;

				break;

			case 3:

				rgb.red = 0xf2d7; rgb.green = 0x0856; rgb.blue = 0x84ec;
			...
			case 5:
				...
				rgb.red = 0x0241; rgb.green = 0xab54; rgb.blue = 0xeaff;

				break;

			case 6:

				rgb.red = 0; rgb.green = 0x8000; rgb.blue = 0x11b0;
...

{
	...
	a = (short double) red;		b = 4294967296; a = a * b; 

	result  = a;

	a = (short double) green;	b = 65536; 		a = a * b; 
...

{
...

		{
...

			{
				...
				if(	WinData[WinNum].status<1 ) { ErrHandler( 1009, "\p in GetPixel function" ); return(-1); }
		...
		ad = (short double) r1.red;		b = 4294967296; ad = ad * b; 

		result  = ad;

		ad = (short double)  r1.green;		b = 65536; 		ad = ad * b; 
...

{
	...
	if( !WinData[a].status ) { ReturnStrLen = 0; return -1;}
...

{
	...
	for( a=0; a<100; a++ )	if( WinData[a].status && window == WinData[a].theWindow ) { b=a; break; }

	if( b == -1 ) { ReturnStrLen = 0; return -1;}
