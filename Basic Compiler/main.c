...
void COD( UCHAR *msg )
{
	...
	TextSize(14);
	a = StringWidth( msg );
	MoveTo(  (404-a)/2,190 );  DrawString(msg);
	//Delay(40, &f);
	return;
...
void compile_error()
{
	...
	error = FSpOpenDF( &source_fp, fsRdWrShPerm, &rn);
	    if(error)  return; 
	error = FSClose( rn);    if(error) return; 
...
short CallTableFind( char *cmdname, short PassCallType )
{
	...
	if( CallType>1000) { CallType-=1000; dupCheckFlag++; }
...
short ObjectTableFind( Str255 objname, short objtype )  // object could include form1.ctl
{
	...
	b=objname[0];   
	for( a=1; a<=b;a++) if( objname[a]=='.' ) dp++; gset=a;
...
short register_sub()
{
	...
	if( a >= 0 ) { compile_log_update(137, nil); return(-2); }
...
short load_source_file( unsigned char * fname)
{
	...
	buffer =  (unsigned char *) malloc( ln+16); 
	 if( buffer == NULL) { FSClose( rn); return(0); }
	error = FSRead( rn, &ln, buffer);
	 if(error) { free(buffer); FSClose( rn); return(0); }
	source_len = ln;                // This lets next_line know file len
	error = FSClose( rn);
	 if(error) { free(buffer); return(0); }
	return(1);
...
short load_libdef( )
{
	...
	if(error) { return(1); }
	error = GetEOF( rn, &ln);
	if(error) { FSClose( rn); return(1); }


    // Allocate buffer for source file
	buffer =  (unsigned char *) malloc( ln+16); 
	 if( buffer == NULL) { FSClose( rn); return(2); }
	error = FSRead( rn, &ln, buffer);
	 if(error) { free(buffer); FSClose( rn); return(1); }
	error = FSClose( rn);
    ...
    if( a ) { lde(2,0); free(buffer); return(3); }
    if( !(buffer[bcc] == '[' &&  buffer[bcc+1] == 'C'  &&  buffer[bcc+2] == 'o'  &&  buffer[bcc+3] == 'm'))
	{ lde(2,0); free(buffer); return(3); }
    a = ld_nextline(ln);
 	if( a ) { lde(1,0); free(buffer); return(3); }
	...
	while(!b)
	{
		...
		ff[0]=c-1; StringToNum( ff, &d );
		if( d<1 ) { lde(5,ldlc); free(buffer); return(3); }
		...
		if( !c ) {  lde(4,ldlc); free(buffer); return(3); }
...
short load_project_list( ) //unsigned char * fname)
{
	...
	zbuf_pos = 0;
	znext_line();
	if( !StrPartCmp( zline,"MacStandardBasic Project" ) ) { SysBeep(30); return -1; }
...
//	if( !a) if( StrPartCmp( zline,"Target" ) )  { zsplit(); StringToNum( zparm2, &bl ); LibraryMode = (short) bl;}
	...
	{ zsplit(); Str255Copy( zparm2, Target ); Str255Copy( zparm2, TargetDef );
	  Str255Cat( "\p.Def", TargetDef ); b=TargetDef[0]; TargetDef[b+1]=0;
	}
	a = znext_line();
	if( !a) if( StrPartCmp( zline,"MemMin" ) )  { zsplit(); StringToNum( zparm2, &bl ); MinMem = (short) bl;}
	a = znext_line();
	if( !a) if( StrPartCmp( zline,"MemPref" ) )  { zsplit(); StringToNum( zparm2, &bl ); PrefMem = (short) bl;}
	a = znext_line();
		...
		while(result)
		{
			...
			if(result)
			{
				...
				else
				{
					...
					pass2_compile_file();
					free( buffer );
					if( !ErrorCtr &&  PjtFileList[pjt_ctr-1].tp==1 )
					{	// Now match up if form
						DoFormCallSync( );			
					}
				}
			}
...
short compile_project(  )
{
	...
	CSWindow = GetNewCWindow( 1000, nil, (WindowPtr) -1); 	if( !CSWindow ) return(-1);
	
	thePic = GetPicture( 301); if( !thePic ) return(-1);
	r = (*thePic) -> picFrame;
	a = r.right-r.left;   b = r.bottom-r.top;
	SetRect( &r, 0,0,a,b);
	OffsetRect( &r, 20, 0);
	...
	TextSize(14);
	MoveTo( 46, 90 );  DrawString("\pVisual MacStandardBasic Compiler 4.1");
	MoveTo(  60,106 );  DrawString("\pCopyright � 2000 Squirrel Software");
	MoveTo( 120,122 );  DrawString("\pAll Rights Reserved");
	//Delay( 400, &z1);
	...
	else
	{
		...
		result2 = add_CallTable_page();   if(!result2) { memory_error(); return(0); }
		result2 = add_VarTable_page();    if(!result2) { memory_error(); return(0); }
		result2 = add_LabelTable_page();  if(!result2) { memory_error(); return(0); }
		result2 = add_CodeTable_page();   if(!result2) { memory_error(); return(0); }
		result2 = add_StringTable_page(); if(!result2) { memory_error(); return(0); }
		result2 = add_SelectTable_page(); if(!result2) { memory_error(); return(0); }
		result2 = add_PropTable_page();   if(!result2) { memory_error(); return(0); }
