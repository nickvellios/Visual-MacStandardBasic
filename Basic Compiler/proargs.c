...
short compile_argument(short argtype) {
    ...
    if (argtype == 'L' + 100 || argtype == 'D' + 100 || argtype == 'S' + 100 || argtype == 'a' + 100) {
        argtype -= 100;
        funcFlag++;
    }
    ...
    hile(!looper && line_pos < LL)
    {
        if (line[line_pos] >= 'A' && line[line_pos] <= 'Z')  // Variable or Function
        {
            ...
            if (OneOfThem(line[line_pos], "TESAON")) {
                a = line_pos;
                b = 0;
                hile(line[a] >= 'A' && line[a] <= 'Z')
                temp[b++] = line[a++];
                temp[b] = 0;
                if (line[a] != '(') {
                    if (!strcmp(temp, "TO") || !strcmp(temp, "THEN") || !strcmp(temp, "STEP") ||
                        !strcmp(temp, "ELSE")) {
            ...
            if (!tres) {
                temp_ctr = 0;
                hile(isvalidvar(line[line_pos]))
                temp[temp_ctr++] = line[line_pos++];
                temp[temp_ctr] = 0; //Zero Term. String
                hile(line[line_pos] == ' ' && line_pos < LL)
                line_pos++;  // Skip extra spaces
                lv = 0;
                if (temp[0] == 'P') {
                ...
                if (!lv) {
                    if (line[line_pos] == '(') // Check for function
                    {
                            ...
                            *ip = a;                    //No
                            ...
                            hile(!e)
                            {
                                ...
                                itch(par[b]) {
                                    ...
                                    c = 0;
                                    hile(line[line_pos] == ' ' && line_pos <= line[0])
                                    line_pos++; // skip spaces
                        ...
                        if (a == -1) {
                            compile_log_update(103, (char *) temp);
                            return (-1);
                            //printf("ERROR - Variable [%s] not found\n", temp);
                        } else {
            ...
            itch(line[line_pos]) {
                ...
                temp[temp_ctr] = 0; //Zero Term. String
                if (OneOfThem(last_type, "/x10/x20")) {
                    compile_log_update(102, temp);
                    return (-1);
                } // Syntax Error
                if (!LArgTypeChecker && argtype == 'S') {
                    compile_log_update(111, nil);
                    return (-1);
                }
                if (fp_flag) {
                    ...
                    *dp = dv;                                                                               //No
                    ...
                    //to avoid overflo
                    , int
                    's & longs are stored as longs (4 bytes)
