...
void EditPropValue(Point pt)  // Point is already local
{
    ...
    if (objecttype <= 0) {    // Click in view code button?
        theRect = PropWin->portRect;
        OffsetRect(&theRect, -theRect.left, -theRect.top);
        a = theRect.right - 82;
    ...
    switch (propSel) {
        ...
        case 10: // Fontname
            ...
            for (v1 = 1; v1 <= cc; v1++) {
                ...
                if (!objecttype)
                    if (EqualString(FormProps[CurrentForm].fontname, text1, false, false)) {
                        ctr = v1;
                        break;
                    } else if (EqualString(ObjectProps[objnum].fontname, text1, false, false)) {
                        ctr = v1;
                        break;
                    }
            }
            ...
            if (!objecttype) {
                ...
                if (objecttype == 3) // Textbox scrollbar
                {
                    ...
                    {   // Yes add it
                        ...
                        CtlData[objnum].ctl = NewControl(FormWin[CurrentForm], &tr3, "\p", 1, 0, 0, 0, myscrollBarProc,
                                                         0);
...
void PropDispose() {
    ...
    tr.left = 0;
    tr.right = PropWin->portRect.right - 15;
        ...
        case 5:
            ...
            if (v1 >= -32767 && v1 <= 32767) {
                if (!objecttype) {
                    ...
                    SizeWindow(FormWin[CurrentForm], FormProps[CurrentForm].width, FormProps[CurrentForm].height,
                               false);
                } else {
                    ...
                    case 5:  // Picture
                        if (ObjectProps[objnum].value > 0) {
                            ...
                            for (z2 = 1; z2 <= z1; z2++) {
                                ...
                                if (rf != -1) {
                                    UseResFile(rf);
                                    if (objecttype == 4) {   //icon
                                        d = PlotIconID(&CtlData[objnum].theRect, atAbsoluteCenter, ttNone,
                                                       ObjectProps[objnum].value);
                                        if (!d) {
                                        ...
                                        CtlData[objnum].pic = (PicHandle) Get1Resource('PICT',
                                                                                       ObjectProps[objnum].value);
                                    if (CtlData[objnum].pic != nil) {
...
short LoadFormContents(short formNum) {
    ...
    while (!a) {
        if (EqualString(zparm1, "\pControlBegin", false, false)) {
            for (c = 1; c <= 11; c++)
                if (EqualString(zparm2, objectNames[c], false, false)) {
                    g = c;
                    break;
                }
            if (f == -1) return -2;
            ...
            CtlData[f].pic = nil;
            CtlData[f].status = g;
            CtlData[f].WinNum = formNum;
            CtlData[f].lastItem = 0;
            while (!a) {
                a = znext_line();
                zsplit();
                //printf(">>%s %s\n", zparm1+1, zparm2+1 );
...
void StoreFormValue(short dd, short objecttype) {
    ...
    if (!objecttype) {
        for (v1 = 0; v1 <= 22; v1++)
            if (EqualString(zparm1, FormPropsDesc[v1], false, false)) {
                propnum = v1;
                break;
            }
    } else {
        for (v1 = 0; v1 <= 22; v1++)
            if (EqualString(zparm1, CtlPropsDesc[v1], false, false)) {
                propnum = v1;
                break;
            }
    }
    ...
    switch (propnum) {
        ...
        case 2:
            ...
            if (!objecttype) FormProps[dd].left = v1;
                ...
                SetRect(&CtlData[objnum].theRect, ObjectProps[objnum].left, ObjectProps[objnum].top,
                        ObjectProps[objnum].left + ObjectProps[objnum].width,
                        ObjectProps[objnum].top + ObjectProps[objnum].height);
            }
            break;
        case 3:
            StringToNum(zparm2, &t);
        ...
        case 21:
            ...
            text1[0] = v2 - 1;
            StringToNum(text1, &t);
            blue1.red = (unsigned short) t;
            v1++;
            v2 = 1;
            while (v1 <= zparm2[0] && zparm2[v1] != ' ') text1[v2++] = zparm2[v1++];
            text1[0] = v2 - 1;
            StringToNum(text1, &t);
            blue1.green = (unsigned short) t;
            v1++;
            v2 = 1;
            while (v1 <= zparm2[0] && zparm2[v1] != ' ') text1[v2++] = zparm2[v1++];
            text1[0] = v2 - 1;
            StringToNum(text1, &t);
            blue1.blue = (unsigned short) t;
    ...
    for (c = 0; c < 200; c++) {
        if (CtlData[c].status && CtlData[c].WinNum == formNum) {
            switch (CtlData[c].status) {
                case 1: // Button
                    CtlData[c].ctl = NewControl(window, &CtlData[c].theRect, ObjectProps[c].text, 1, 0, 0,
                                                0, /*pushButProc*/0, 0);
                    break;
                case 2: // label
                case 3: // TextBox
                    ...
                    if (CtlData[c].te != nil) {
                        ...
                        if (ObjectProps[c].opt14) {   // Scrollbar
                            tr1 = CtlData[c].theRect;
                ...
                case 5: // Picture
                    ...
                    break;
                case 6:    // CheckBox
                    CtlData[c].ctl = NewControl(window, &CtlData[c].theRect, ObjectProps[c].text, 1,
                                                ObjectProps[c].value, 0, 1, /*checkBoxProc*/369, 0);
                    break;
                case 7:    // Radio
                    CtlData[c].ctl = NewControl(window, &CtlData[c].theRect, ObjectProps[c].text, 1,
                                                ObjectProps[c].value, 0, 1, /*radioButProc*/370, 0);
                    break;
                case 8:    // DropList
                    cn = c + 10000;
                    ...
                    CtlData[c].ctl = NewControl(window, &CtlData[c].theRect, ObjectProps[c].name, true, 0, cn,
                                                0,/*popupMenuProc*/400, 0);
                    SetControlMinimum(CtlData[c].ctl, 1);
                    SetControlMaximum(CtlData[c].ctl, 2);
                    SetControlValue(CtlData[c].ctl, 1);
                    break;
                ...
                case 10:    // Timer
                    ...
                    break;
                case 11:    // Scrollbar
                    CtlData[c].ctl = NewControl(window, &CtlData[c].theRect, "\p", 1, ObjectProps[c].value,
                                                ObjectProps[c].opt17, ObjectProps[c].opt18, myscrollBarProc, 0);
                    break;
            }
        }
    }
