...
short CreateFormWindow() {
    ...
    FormProps[c].opt15 = 1;
    FormProps[c].opt16 = 8;
    FormProps[c].opt17 = 100;
    FormProps[c].opt18 = 2000;
    FormProps[c].opt19 = 100;
    FormProps[c].opt20 = 2000;
    FormProps[c].opt21 = 0;
...
short RecreateFormWindow(short c) {
    ...
    if (c == -1) return (-1);
    SetRect(&tempRect, FormProps[c].left, FormProps[c].top, FormProps[c].left + FormProps[c].width,
            FormProps[c].top + FormProps[c].height);
    w = FormProps[c].value;
    if (w == 9 || w >= 11)
        FormWin[c] = NewCWindow(nil, &tempRect, FormProps[c].text, false, wt[w - 1], (WindowPtr)(-1), false, 0);
        ...
        if (CtlData[a].status && CtlData[a].WinNum == c) {
            switch (CtlData[a].status) {
                ...
                case 5:
                    ...
                    if (CtlData[a].pic != nil) {
                        KillPicture(CtlData[a].pic);
                        CtlData[a].pic = nil;
                    }
                    break;
                case 10:
