...
void FormEditorCall( short ctlNum)
{
	...
	for( a=0;a<100;a++) 
	{
		if( EditWin[a].theWindow!=nil && CurrentForm == EditWin[a].FormNum ) {b =a; break;}
	}
	if( b>=0 )
	{
		SelectWindow( EditWin[b].theWindow );
...
void LoadEdit( short ctlNum, short b)
{
	...
	else
	{
		for( c=4; c<=noi; c++)
		{
			...
			if( EqualString( text2, text1, false, false ) )  { createFlag = 0; break; }
		}
	}
	if( createFlag )
	{
...
short SaveEditBox( short b)
{
	...
	{
		if( EditWin[b].editFlag == 0 ) { {SysBeep(30); dprintf("Error @ " __FILE__" l. %d\n", __LINE__);} return 1;}
...
short DoEditCheck(  short b )
{
	...
	if( StrPartCmp( text1, "Sub " ))
	{
		...
		*((*chnd)) = 'S'; 	*((*chnd)+1) = 'u';  *((*chnd)+2) = 'b';
		...
		else
		{
			...
			a=1;
			c = 4;
			while( *((*chnd)+c)!='(' ) { text2[a++] = *((*chnd)+c);  c++; } // Copy proc name to str255
			text2[0] = a-1;
		}
	}
	else if( StrPartCmp( text1, "Function " ))
	{
		ct=2;
		// Insure proper caps
		*((*chnd)) = 'F'; 	*((*chnd)+1) = 'u';  *((*chnd)+2) = 'n'; *((*chnd)+3) = 'c';
		*((*chnd)+4) = 't'; *((*chnd)+5) = 'i';  *((*chnd)+6) = 'o'; *((*chnd)+7) = 'n';
		a = 9;
		while( *((*chnd)+a) == ' ' ) a++;
		if( a != 9 ) 
		{
...
void TextFix( WindowPtr window )
{
	...
	cnum=-1;
	for(a=0; a<100;a++ ) if( EditWin[a].status && EditWin[a].theWindow == window ) { cnum = a; break;}
	if( cnum == -1 ) return;
...
short CreateEditorWindow( short createFlag, Str255 title )
{
	...
	if( CurrentForm==-1 )
	{
		for(a=20;a<100;a++) if( !EditWin[a].status ) { b=a; break; }
		if( b==-1 ) return -1;
		if( createFlag )
		{
			ModuleCtr++;
	...
	//EditWin[b].te = TENew( &tr, &tr); 
	if( EditWin[b].te==nil ) { IDEmemErr(); return -1; }
	// Adjust
	EditWin[b].ht = TEGetHeight( 0, 0, EditWin[b].te );
	if( !EditWin[b].ht ) { {SysBeep(30); dprintf("Error @ " __FILE__" l. %d\n", __LINE__);}}
	...
	OffsetRect( &theRect, -theRect.left, -theRect.top);
	theRect.top =31; theRect.bottom-=14;
	theRect.right++;
	...
	rgb.red = 0xb000; rgb.green = 0xb000; rgb.blue = 0xb000;
	RGBForeColor( &rgb );
	...
	EditWin[b].dropList = NewControl( EditWin[b].theWindow, &tempRect, "\p", false, 0, 11000+b ,0,/*popupMenuProc*/400, 0); 
	...
	for(a=0;a<100;a++)
	{
		if( EditWin[a].status && EditWin[a].theWindow == window )
		{
			...
			BeginUpdate(window);
				...
				rgb.red = 0xd000; rgb.green = 0xd000; rgb.blue = 0xd000;
				RGBForeColor( &rgb );
				MoveTo( 0, 30);  LineTo( 0, 0 );    LineTo( b, 0 );
				MoveTo( 1, 29);  LineTo( 1, 1 );    LineTo( b-1, 1 );

				rgb.red = 0x7fff; rgb.green = 0x7fff; rgb.blue = 0x7fff;
				RGBForeColor( &rgb );
				MoveTo( 1, 29);  LineTo( b-1, 29 ); LineTo( b-1, 0 );
				MoveTo( 0, 30);  LineTo( b, 30 );   LineTo( b, 0 );
...
short DoEditGrow( Point hitPt, WindowRef window )
{
	...
	if(a>=0)
	{
		...
		OffsetRect( &theRect, -theRect.left, -theRect.top);
		theRect.top =31; theRect.bottom-=15;
...
short CloseEditWindow( WindowPtr window)
{
	...
	if( a>=20 )
	{	// This is a module so check for save
		if( EditWin[a].change )
		{
...
short CheckForEdit(WindowPtr window)
{
	...
	for(a=0;a<100;a++) if( EditWin[a].status && EditWin[a].theWindow==window ) { b=a; break; }
	if( b==-1 ) return -1;
...
void MarkChangeTE( )
{
	...
	for( a=0; a<100; a++)
		if(	ActiveTE == EditWin[a].te ) { EditWin[a].change=1; break; }
}
...
short SaveSourceFile( WindowPtr window )
{
	...
	if( CurrentForm ==-1 )
	{
		...
		for( a=20;a<100;a++) 
		{
			if( EditWin[a].status && window == EditWin[a].theWindow ) { dd = a; break;}
		}
		if( dd==-1 ) { {SysBeep(30); dprintf("Error @ " __FILE__" l. %d\n", __LINE__);}return -1; }
	}
	else
	{
		dd = CurrentForm;
					...
					else
					{
						...
						if( EditWin[dd].status <2 )
						{
							...
							for( a=1; a<=noi; a++)
							{
								...
								if ( EqualString( text1, EditWin[dd].FileName, false, false ) ) { z=1; break; }
							}			
		...
		if( EditWin[dd].status < 2  || SaveAsFlag )
		{
			while( !looper )
			{
				...
				{	
					...
					{
						//{SysBeep(30); dprintf("Error @ " __FILE__" l. %d", __LINE__);}
						theDialog = GetNewDialog( 280, nil, (WindowPtr) -1L );						ModalDialog( 0, &cc );
						DisposeDialog( theDialog);
					}
					else
					{
	...
	if( dd<20 ) { bl=35; FSWrite( fref, &bl, hdr1 );	}  // Write header
	else { bl=37; FSWrite( fref, &bl, hdr2 );	   		}// Write header
	...
	if(  EditWin[dd].GlobalVar != nil )
	{
		...
		bl = 7;	FSWrite( fref, &bl, "Global\x0d" );	   
		cp = (*EditWin[dd].GlobalVar);
		bl=EditWin[dd].GlobalVarSize;
		FSWrite( fref, &bl, cp );	  
		bl = 1; 	FSWrite( fref, &bl, cr );	   // Add extra CR
		bl = 11;	FSWrite( fref, &bl, "EndGlobal\x0d\x0d" );	   
...
short LoadSourceFile( short fddi, Str255 fname )
{
	...
	{
		case 1:  // Form
			for( a=0;a<20;a++) if(FormWin[a]==nil ) {dd=a; break;}		
			if( dd == -1 ) return -1;	
	...
	{
		//printf("~%s\n", zline );
		if( StrPartCmp( zline, "Start" ) || StrPartCmp( zline, "Sub " ) || StrPartCmp( zline, "Function " ) || StrPartCmp( zline, "Global" ) )
		{
			if( StrPartCmp( zline, "Global" ) )  {	t1 = zbuf_pos;   a = znext_line();    }
			else  { t1 = ztstart; 	a = znext_line(); }  // Start, Sub and Func
			b = 0;
			while( !b)
			{
					//printf("~  %s\n", zline );
