                ...
                if (IsInGrow(a, event->where)) {
                    ...
                    if (a >= 0) {
                        if (currTool == 1) {
                            ...
                            StretchBox(window);
                            if (MasterRect.bottom - MasterRect.top < 10 || MasterRect.right - MasterRect.left < 10) {
                                {
                                    SysBeep(30);
                            ...
                            switch (currTool) {
                                case 2: // Button
                                    CtlData[ct].ctl = NewControl(window, &MasterRect, "\p", 1, 0, 0, 0, mypushButProc,
                                                                 0);
                                ...
                                case 6: // Picture
                                    ...
                                    break;
                                case 7:    // CheckBox
                                    CtlData[ct].ctl = NewControl(window, &MasterRect, "\p", 1, 0, 0, 1, /*checkBoxProc*/
                                                                 369, 0);
                                    if (CtlData[ct].ctl != nil) {
                                        CtlData[ct].status = 6;
                                        CtlData[ct].WinNum = a;
                                    ...
                                    break;
                                case 8:    // Radio
                                    CtlData[ct].ctl = NewControl(window, &MasterRect, "\p", 1, 0, 0, 1, /*radioButProc*/
                                                                 370, 0);
                                    if (CtlData[ct].ctl != nil) {
                                        CtlData[ct].status = 7;
                                        CtlData[ct].WinNum = a;
                                ...
                                case 9:    // DropList
                                    ...
                                    MasterRect.bottom = MasterRect.top + 20;
                                    CtlData[ct].ctl = NewControl(window, &MasterRect, "\p", true, 0, cn,
                                ...
                                case 10:    // ListBox
                                    ...
                                    CellSz.h = 0;
                                    CellSz.v = 0;
                                    CtlData[ct].lt = LNew(&MasterRect, &theRect, CellSz, 0, window, true, false, false,
                                                          true);
                                ...
                                case 11:    // Timer
                                    ...
                                    break;
                                case 12:    // Scrollbar
                                    CtlData[ct].ctl = NewControl(window, &MasterRect, "\p", 1, 0, 0,
                                                                 0, /*scrollBarProc*/16, 0);
                                    if (CtlData[ct].ctl != nil) {
            ...
            if (ActiveTE == PropTE) {
                if (theChar == 127) {
                    if (((*ActiveTE)->selStart == (*ActiveTE)->selEnd) &&
                        ((*ActiveTE)->selStart < (*ActiveTE)->teLength)) {
                        ((*ActiveTE)->selStart)++;
                        ((*ActiveTE)->selEnd)++;
                        TEKey(8, ActiveTE);
                    ...
                    if ((*ActiveTE)->teLength >= 255 && theChar != 8 && theChar != 28 && theChar != 29) {
                        SysBeep(30);
                        dprintf("Error @ " __FILE__" l. %d\n", __LINE__);
                    } else {
...
HandleOpenDocument(const AppleEvent *ae, AppleEvent *reply, long refCon) {
...
HandleApplicationDied(const AppleEvent *ae, AppleEvent *reply, long refCon) {
#pragma unused ( reply, refCon )
    OSErr err;
    // perform the recommended check for additional required parameters
...
HandleOpenApplication(const AppleEvent *ae, AppleEvent *reply, long refCon) {
#pragma unused ( reply, refCon )
    OSErr err;
    // perform the recommended check for additional required parameters
    if ((err = GotRequiredParams(ae)) != noErr)
...
HandleQuitApplication(const AppleEvent *ae, AppleEvent *reply, long refCon) {
#pragma unused (reply, refCon)
    AEKeyword optKey;
    DescType actualType;
    Size actualSize;
...
OSErr InitializeEvents(void) {
    ...
    if ((err = AEInstallEventHandler(kCoreEventClass, kAEApplicationDied, NewAEEventHandlerProc(HandleApplicationDied),
                                     0, false)) != noErr)
        goto cleanup;
    if ((err = AEInstallEventHandler(kCoreEventClass, kAEOpenApplication, NewAEEventHandlerProc(HandleOpenApplication),
                                     0, false)) != noErr)
        goto cleanup;
    ...
    if ((err = AEInstallEventHandler(kCoreEventClass, kAEPrintDocuments, NewAEEventHandlerProc(HandleOpenDocument),
        ...
        goto cleanup;
    if ((err = AEInstallEventHandler(kCoreEventClass, kAEQuitApplication, NewAEEventHandlerProc(HandleQuitApplication),
                                     0, false)) != noErr)
        goto cleanup;
