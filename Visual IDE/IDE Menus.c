    ...
    while (!looper) {
        ...
        if (reply.sfGood) {
            if (reply.sfFile.vRefNum != PjtFS.vRefNum || reply.sfFile.parID != PjtFS.parID) {
                ...
                error = FSpOpenDF(&reply.sfFile, fsRdWrShPerm, &cc);
                if (error) { return; }
                error = GetEOF(cc, &FLEN);
                if (error) {
                    FSClose(cc);
                    return;
                }
                if (!FLEN) {    // Resource
                    ...
                    if (StrPartCmp(lbuf, "MacStandardBasic Form")) {    // module
            ...
            case 1: // Replace
                ...
                if (nFindText[0]) {
                    ...
                    if (value == 1) {
                        ...
                        SetPort(gp);
                        TESelView(ActiveTE);
                        if (FindOptions[1] == '0') {    // FInd & Verify
    ...
    switch (menuItem) {
        ...
        case kItemPaste:
            ...
            if (ActiveTE != nil && TEGetScrapLength()) {
                ...
                b = TEGetScrapLength();
                TEPaste(ActiveTE);
                if (undoCode == 3) {    // put old selected text in local scrap
                    TESetSelect(undoStart, undoEnd, ActiveTE);
                    TECut(ActiveTE);
                }
                undoEnd = undoStart + b;
