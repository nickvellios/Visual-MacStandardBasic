    ...
    if (!a)
        if (StrPartCmp(zline, "Target")) {
            ...
            if (Target[0] == 1) // probably so
            {
                if (Target[1] == '1') {
                    marker = 1;
                    Str255Copy("\pStandard 68K Library", Target);
                    ProjectFlag = 1;
                }
                if (Target[1] == '2') {
                    marker = 1;
                    Str255Copy("\pStandard PPC Library", Target);
                    ProjectFlag = 1;
                }
                if (Target[1] == '3') {
                    marker = 1;
...
void PrintStuff(short ModNum, short EdProc) {
    ...
    if (EdProc >= 0) {
        ...
        if (EditWin[ModNum].GlobalVar != nil) {    // Do Global first
            hPrintText = EditWin[ModNum].GlobalVar;
            PrintTextSize = EditWin[ModNum].GlobalVarSize;
            gFlag = 1;
        } else {
